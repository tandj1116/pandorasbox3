...
{
	public class S2_BoxFolder : TSWizards.BaseInteriorStep
	{
		...
		public S2_BoxFolder()
		{
			...
			this.Description.Text = "Please select the desitnation folder for the BoxServer scripts.";
		}
		...
		private void button1_Click(object sender, System.EventArgs e)
		{
			if ( FolderBrowser.ShowDialog() == DialogResult.OK )
			{
				...
				string scripts = Path.Combine( Setup.RunUOFolder, "Scripts" );

				if ( Setup.BoxFolder.IndexOf( scripts ) == -1 )
				{
					MessageBox.Show( "The BoxServer folder must be located within the scripts folder of the RunUO installation specified in the previous step." );
				...
				{
					if ( MessageBox.Show( this, "The selected folder already exists. Files within this folder might be overwritten. Are you sure you wish to proceed?", "Folder already exists", MessageBoxButtons.YesNo, MessageBoxIcon.Warning )
						== DialogResult.No )
					{
						Setup.BoxFolder = null;
					}
		...
		private void S2_BoxFolder_ValidateStep(object sender, System.ComponentModel.CancelEventArgs e)
		{
